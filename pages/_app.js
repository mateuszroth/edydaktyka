...
import { Layout, Menu, Breadcrumb, Icon, LocaleProvider, BackTop } from 'antd'
import plPL from 'antd/lib/locale-provider/pl_PL'
import AuthProvider, { AuthConsumer } from '../components/providers/AuthProvider'
              ...
              <Header className={styles.header}>
                  <AuthConsumer>
                    ...
                    >
                      ...
                      <Menu.Item key="4"><Link href="/articles"><a><Icon type="folder-open" />Popularyzatorskie</a></Link></Menu.Item>
                      ...
                      {!auth.loggedIn && <Menu.Item key="6"><Link href="/questionnaire"><a><Icon type="book" />Ankieta</a></Link></Menu.Item>}
                      {!auth.loggedIn && <Menu.Item key="7"><Link href="/register"><a><Icon type="user-add" />Rejestracja</a></Link></Menu.Item>}
                      {!auth.loggedIn && <Menu.Item key="8"><Link href="/login"><a><Icon type="login" />Logowanie</a></Link></Menu.Item>}
                      {/* tylko dla niezalogowanych */}

                      {/* tylko dla zalogowanych */}
                      {auth.loggedIn && <Menu.Item key="9"><Link href="/schedule"><a><Icon type="schedule" />Plan</a></Link></Menu.Item>}
                      {auth.loggedIn && <Menu.Item key="10"><Link href="/team"><a><Icon type="team" />Zespół</a></Link></Menu.Item>}
                      {auth.loggedIn && <Menu.Item key="11"><Link href="/account"><a><Icon type="smile" />Twoje konto</a></Link></Menu.Item>}
                      {/* tylko dla zalogowanych */}
                    </Menu>
                  )}
                </AuthConsumer>
