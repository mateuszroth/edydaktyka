...
const GroupRegisterForm: React.FC<GroupRegisterFormProps> = ({ form }) => {
  ...
  const [assignUserToGroups, { data, error }] = useMutation(
  ...
  const userGroupIds =
    (authState.user &&
      ...
      authState.user.groups.map(group => group.id)) ||
    [];
  const notAssignedGroups =
    groupsData &&
    groupsData.groups &&
    groupsData.groups
      .map(group => (!userGroupIds.includes(group.id) ? group : null))
        ...
        <>
          <Form.Item label="Grupy zajęciowe:">
            {getFieldDecorator("groups", {
              rules: [
                { required: true, message: "Wybierz chociaż jedną grupę" }
              ]
            })(
...
const WrappedGroupRegisterForm = Form.create<GroupRegisterFormProps>()(
  GroupRegisterForm
);
