...
const Footer: React.FC<{}> = () => {
  ...
  const isNight =
    location &&
    (location.sunrise > location.currentDate ||
      location.sunset < location.currentDate);
  const isDay =
    location &&
    location.sunrise <= location.currentDate &&
      location.sunset >= location.currentDate;
  ...
  const christmasStart =
    location && new Date(`${location.currentDate.getFullYear()}-12-01`);
  const christmasEnd =
    location && new Date(`${location.currentDate.getFullYear()}-01-31`);
  const isChristmasTime =
    location &&
    (location.currentDate <= christmasEnd ||
      location.currentDate >= christmasStart);
      ...
      <div className={classNames(styles.footerAnnotation, { [styles.christmas]: isChristmasTime })}>
        ...
        {location && location.sunrise && (
          <>
            ...
            {location.sunrise.toLocaleTimeString()} i{" "}
            {isAfterSunset ? "zaszło" : "zachodzi"} o{" "}
            {location.sunset.toLocaleTimeString()}, a porę dnia odwzierciedla
            obrazek słońca lub księżyca w tle. <br />
            Poza tym od 1 grudnia do 31 stycznia pojawia się motyw świąteczny. Icons in background made by Smashicons &amp; Freepik from
            www.flaticon.com.
          </>
        )}
